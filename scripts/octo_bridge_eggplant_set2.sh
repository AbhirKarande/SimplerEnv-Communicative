...
LOG_DIR="$LOG_ROOT/simpler_env_mc_data_dec27"  # SAME DIRECTORY as Set 1
...
echo "=========================================="
echo "MC DATA COLLECTION: PutEggplant SET 2 (Episodes 28-49)"
...
do CUDA_VISIBLE_DEVICES=${gpu_id} python -m simpler_env.main_inference --policy-model ${policy_model} --ckpt-path ${ckpt_path} \
  ...
  --additional-env-save-tags octo_init_rng_${init_rng}_mc${MC_PASSES}_set2 \
...
echo "=========================================="
echo "PutEggplant MC data collection SET 2 COMPLETED"
