...
do CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference.py --policy-model ${policy_model} --ckpt-path None \
  ...
  --robot-init-x 0.35 0.35 1 --robot-init-y 0.21 0.21 1 --obj-variation-mode episode --obj-episode-range 28 56 \
  ...
  --instruction-refine-procedure ${INSTR_PROC} --instruction-refine-task ${INSTR_TASK} \
  --mc-logging \
